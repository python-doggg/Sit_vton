...
class Transport:
    ...
    def training_losses(
        ...
        t, x0, x1 = self.sample(x1) # 获取与x1相关的时间t、初始状态x0和目标状态x1
        t, xt, ut = self.path_sampler.plan(t, x0, x1) # 使用self.path_sampler.plan方法规划从x0到x1的路径，返回时间t、采样状态xt和真实速度ut
        model_output = model(xt, t, **model_kwargs)
        B, *_, C = xt.shape
        #print("model_output.size()", model_output.size())
        #print("(B, *xt.size()[1:-1], C)", (B, *xt.size()[1:-1], C))
