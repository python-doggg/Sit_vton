...
def main(args):
    ...
    # Setup DDP:
    dist.init_process_group("nccl") #dist.init_process_group("nccl") # 用于初始化多GPU训练过程中的进程组
    ...
    model = DDP(model.to(device), device_ids=[rank], find_unused_parameters=find_unused_parameters) # DDP 允许你将一个模型复制到多个 GPU 上，每个 GPU 处理模型的一部分
