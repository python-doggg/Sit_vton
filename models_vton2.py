...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, x, cond, mask=None):
        ...
        B, N, C = x.shape # B是批量大小，N是每个样本的元素数量，C是每个元素的特征数量
...
class SiTBlock(nn.Module):
    ...
    def forward(self, x, y, c, mask=None):
        ...
        x = x + gate_msa * self.attn(modulate(self.norm1(x), shift_msa, scale_msa)) # yuan gate_msa.unsqueeze(1)
        x = x + self.cross_attn(x, y, mask)
        x = x + gate_mlp * self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp)) # yuan gate_mlp.unsqueeze(1)
        return x
